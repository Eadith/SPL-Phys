...
def _calculate_fft_hr(ppg_signal, fs=60, low_pass=0.66, high_pass=3):
    """Calculate heart rate based on PPG using Fast Fourier transform (FFT)."""
    ppg_signal = np.expand_dims(ppg_signal, 0)
    N = _next_power_of_2(ppg_signal.shape[1])
    f_ppg, pxx_ppg = scipy.signal.periodogram(ppg_signal, fs=fs, nfft=5400, detrend=False)
...
def calculate_metric_per_video(predictions, fs=30, diff_flag=False, use_bandpass=True, hr_method='FFT'):
