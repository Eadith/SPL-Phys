...
def MMSE_split_percentage(k=5, idx=0):
    ...
    all_files_list = glob.glob('/data/liqi_iai/MMSE-HR/*.h5')
...
# def VIPL_split_percentage(k=5, idx=0):

#     sub_num = list(range(1,108))

#     sub = np.array(['p%d'%n for n in sub_num])
...
#     all_files_list = glob.glob('/data/liqi_iai/VIPL_V1/*.h5')
    
#     train_list = []
#     val_list = []

#     for f_name in all_files_list:
#         sub = f_name.split('/')[-1].split('_')[0]
#         with h5py.File(f_name, 'r') as f:
#             img_len = f['video'].shape[0]
#         if img_len <= 300:
#             continue
#         if sub in sub_val:
#             val_list.append(f_name)
...
#     return train_list, val_list
def VIPL_split_percentage(k=5, idx=1):

    assert idx in [1,2,3,4,5], print('idx error!')
    
    mat_data = scipy.io.loadmat('/root/vipl_folds/fold%d.mat'%idx)
    
    sub_num = mat_data['fold%d'%idx]
    sub_val = np.array(['p%d'%n for n in sub_num[0]])
    
    all_files_list = glob.glob('/root/autodl-tmp/vipl_h5/*.h5')
...
class H5Dataset_(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(h5_f, 'r') as f:
            img_length = np.min([f['video'].shape[0], f['bvp'].shape[0]])
...
class H5Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(self.train_list[idx], 'r') as f:
            ...
            bvp = bvp[idx_start:idx_end].astype('float32')

            img_seq = f['video'][idx_start:idx_end]
            img_seq = np.transpose(img_seq, (3, 0, 1, 2)).astype('float32')
        return img_seq, bvp, label_flag, fps
